...
const corsHeaders = {
  "Access-Control-Allow-Origin": Deno.env.get("PANEL_URL") ?? "*",
...
Deno.serve(async (request) => {
  ...
  if (!profile?.active || profile.role !== "receptionist") return json({ error: "Apenas a recepção pode gerar cobranças." }, 403);

  try {
    const body = await request.json();
    const bookingId = Number(body.bookingId);
    if (!Number.isInteger(bookingId) || bookingId <= 0) return json({ error: "Agendamento inválido." }, 400);
    ...
    const { data: booking, error: bookingError } = await supabase
      ...
      .eq("id", bookingId)
      .single();
    if (bookingError || !booking) return json({ error: "Agendamento não encontrado." }, 404);
    if (!["confirmed", "rescheduled", "completed"].includes(booking.status)) return json({ error: "O pagamento do sinal precisa estar confirmado." }, 409);

    const lead = firstRelation(booking.leads);
    const service = firstRelation(booking.services);
    if (!lead?.email) return json({ error: "A cliente precisa ter um e-mail cadastrado." }, 422);
    ...
    if (outstandingCents <= 0) return json({ error: "Este atendimento já está totalmente pago." }, 409);
    ...
    if (reusable?.checkout_url) return json({ checkoutUrl: reusable.checkout_url, amountCents: outstandingCents, reused: true });
    ...
    if (!accessToken) {
      ...
      return json({ error: "Mercado Pago ainda não configurado." }, 503);
    }
    ...
    return json({ checkoutUrl: preference.init_point, amountCents: outstandingCents }, 201);
  } catch (error) {
    console.error(error);
    return json({ error: "Erro interno ao gerar a cobrança." }, 500);
  }
});
