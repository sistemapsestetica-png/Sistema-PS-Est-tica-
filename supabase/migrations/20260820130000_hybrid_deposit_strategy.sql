-- Estratégia híbrida: 10% do procedimento, mínimo de R$ 30 e máximo de R$ 100.
...
as $$
  select least(
    greatest(
      ceil((p_price_cents * coalesce(p_deposit_percent, cs.deposit_percent, 10))::numeric / 100)::bigint,
      coalesce(p_service_min_deposit_cents, 0),
      coalesce(cs.min_deposit_cents, 3000)
    ),
    coalesce(cs.max_deposit_cents, 10000)
  )
