  ...
  return <main className={s.page}>
    ...
    <section className={s.quizSection} id="diagnostico">
      ...
      <div className={s.quizCard} aria-live="polite">
        ...
        {step === 0 && <div className={s.quizBody}><p className={s.quizKicker}>Indicação inicial personalizada</p><h3>Vamos entender o que você deseja melhorar?</h3><p className={s.quizText}>Não existem respostas certas. Escolha a opção que mais representa o seu objetivo hoje.</p><button className={s.primary} onClick={() => setStep(1)}>Começar agora <span>›</span></button></div>}
        {current && <div className={s.quizBody}><p className={s.quizKicker}>{current.eyebrow}</p><h3>{current.title}</h3><div className={s.options}>{current.options.map((option) => <button key={option} className={answers[current.key] === option ? s.selected : ""} onClick={() => choose(option)}><span>{option}</span><i>›</i></button>)}</div>{step > 1 && <button className={s.back} onClick={() => setStep((step - 1) as Step)}>Voltar</button>}</div>}
