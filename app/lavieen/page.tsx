...
export default function LavieenPage() {
  ...
  const progress = step === 0 ? 0 : step <= 5 ? step * 16 : step === 6 ? 90 : 100;
  ...
  const whatsapp = useMemo(() => {
    const location = city ? ` Sou de ${city}.` : "";
    const message = booking ? `Olá! Sou ${name}.${location} Fiz o quiz Lavieen e pré-reservei ${formatSlot(booking.starts_at)}. Quero confirmar os detalhes.` : `Olá! Sou ${name || "cliente"}.${location} Fiz o quiz Lavieen da PS Estética e quero falar sobre uma avaliação.`;
  ...
  function source() {
    ...
    return { landing_page: "lavieen", quiz_answers: { ...answers, city }, utm_source: p.get("utm_source"), utm_medium: p.get("utm_medium"), utm_campaign: p.get("utm_campaign"), utm_content: p.get("utm_content"), utm_term: p.get("utm_term"), referrer: document.referrer || null, fbp: cookie("_fbp"), fbc: cookie("_fbc") };
  }
  ...
  async function saveLead(withEmail = false) {
    ...
    const { data, error } = await supabase.rpc("capture_lead_session_v2", {
      p_session_token: getLeadSessionToken(), p_name: name.trim(), p_phone: whatsappDigits(phone), p_service_slug: "lavieen",
      p_experience: answers.treatment === "Nunca" ? "primeira" : "ja_fiz", p_timing: "pesquisando",
      p_city: city, p_source: source(), p_email: withEmail ? email.trim().toLowerCase() : null,
    });
  ...
  async function submitContact(event: FormEvent<HTMLFormElement>) {
    ...
    if (loading) return;
    if (city.length < 2) { setNotice("Informe sua cidade para continuar."); return; }
  ...
  return <main className={s.page}>
    ...
    <section className={s.quizSection} id="diagnostico">
      <div className={s.quizIntro}><p className={s.eyebrow}>Seu quiz Lavieen</p><h2>Cinco respostas para começar uma conversa mais objetiva.</h2><p>Leva cerca de um minuto. Ao final, você poderá consultar a disponibilidade da PS Estética.</p></div>
      <div className={s.quizCard} aria-live="polite">
        <div className={s.quizTop}><span>{step === 0 ? "Pronto para começar" : step <= 5 ? `Pergunta ${step} de 5` : step === 6 ? "Seus dados" : step === 7 ? "Indicação e agenda" : "Pré-reserva"}</span><b>{progress}%</b></div><div className={s.progress}><span style={{ width: `${progress}%` }} /></div>
