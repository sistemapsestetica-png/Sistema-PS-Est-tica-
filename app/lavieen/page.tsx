...
export default function LavieenPage() {
  ...
  function source() {
    ...
    return { landing_page: "lavieen", quiz_answers: answers, utm_source: p.get("utm_source"), utm_medium: p.get("utm_medium"), utm_campaign: p.get("utm_campaign"), utm_content: p.get("utm_content"), utm_term: p.get("utm_term"), referrer: document.referrer || null, fbp: cookie("_fbp"), fbc: cookie("_fbc") };
  }
  ...
  async function saveLead(withEmail = false) {
    ...
    const { data, error } = await supabase.rpc("capture_lead_session", {
      p_name: name.trim(), p_phone: whatsappDigits(phone), p_service_slug: "lavieen",
