...
export default function ProfessionalPage() {
  ...
  async function authenticate(event: FormEvent) {
    ...
    if (signup) {
      const { data, error } = await supabase.auth.signUp({ email: normalized, password, options: { emailRedirectTo: `${window.location.origin}/profissional` } });
  ...
  return <main className="admin-page professional-page"><header className="admin-header"><div className="admin-header-brand"><Link className="admin-header-logo" href="/"><img src="/ps-estetica-logo-oficial.png" width="246" height="80" alt="PS Estética" /></Link><div><p className="admin-eyebrow">Agenda profissional</p><h1>{profile.full_name}</h1></div></div><div className="admin-account"><span>{services.map((service) => service.name).join(" · ")}</span><button onClick={() => supabase.auth.signOut()}>Sair</button></div></header>
