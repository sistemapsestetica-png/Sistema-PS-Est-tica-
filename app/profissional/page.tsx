...
export default function ProfessionalPage() {
  ...
  async function loadPanel() {
    ...
    const [slotResult, bookingResult] = await Promise.all([
      ...
      supabase.from("bookings").select("id,status,deposit_cents,leads(name,phone),services(name),slots(starts_at),payments!inner(status)").eq("payments.status", "paid").in("status", ["confirmed", "rescheduled", "completed", "no_show"]).order("created_at", { ascending: false }).limit(100),
    ]);
  ...
  return <main className="admin-page professional-page"><header className="admin-header"><div className="admin-header-brand"><Link className="admin-header-logo" href={QUIZ_URL}><img src="/ps-estetica-logo-oficial.png" width="246" height="80" alt="PS Estética" /></Link><div><p className="admin-eyebrow">Agenda profissional</p><h1>{profile.full_name}</h1></div></div><div className="admin-account"><span>{services.map((service) => service.name).join(" · ")}</span><button onClick={() => supabase.auth.signOut()}>Sair</button></div></header>
    ...
    <section className="admin-panel"><div className="panel-heading"><div><p className="admin-eyebrow">Atendimentos</p><h2>Minha agenda</h2></div><p>Somente clientes com pagamento confirmado aparecem aqui. Pré-agendamentos continuam com a recepção.</p></div><div className="table-wrap"><table><thead><tr><th>Cliente</th><th>Serviço</th><th>Data</th><th>Pagamento</th><th>Atendimento</th><th>Contato</th></tr></thead><tbody>{bookings.length === 0 && <tr><td className="empty-state" colSpan={6}>Nenhum atendimento confirmado.</td></tr>}{bookings.map((booking) => <tr key={booking.id}><td><b>{booking.leads?.name}</b><small>{booking.leads?.phone}</small></td><td>{booking.services?.name}</td><td>{booking.slots ? formatDate(booking.slots.starts_at) : "—"}</td><td><span className="payment-pill paid">Pago</span></td><td><select value={booking.status} onChange={(event) => updateBooking(booking.id,event.target.value)}><option value="confirmed">Confirmado</option><option value="rescheduled">Remarcado</option><option value="completed">Concluído</option><option value="no_show">Não compareceu</option></select></td><td>{booking.leads?.phone && <a href={`https://wa.me/55${booking.leads.phone.replace(/^55/,"")}`} target="_blank" rel="noreferrer">WhatsApp ↗</a>}</td></tr>)}</tbody></table></div></section>
  </main>;
}
