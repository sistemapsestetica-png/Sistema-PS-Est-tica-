...
export default function AdminPage() {
  ...
  async function saveService(service: Service) {
    ...
    if ((priceCents !== null && priceCents <= 0) || !service.duration_minutes) {
      setMessage("Informe uma duração válida. O preço pode ficar em branco quando for definido após a avaliação.");
      return;
    }
  ...
  async function createService(event: FormEvent<HTMLFormElement>) {
    ...
    if (name.length < 2 || (priceCents !== null && priceCents <= 0) || !duration || duration < 5) {
      setMessage("Informe nome e duração válidos. O preço é opcional.");
      return;
    }
    setBusy(true);
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    if (settings.fixed_deposit_cents <= 0) return setMessage("O sinal fixo precisa ser maior que zero.");
    ...
    const { error } = await supabase.from("clinic_settings").update({
      fixed_deposit_cents: settings.fixed_deposit_cents,
  ...
  async function saveLeadNotes() {
    ...
    setBusy(false);
    if (error) { setMessage(`Não foi possível salvar as observações: ${error.message}`); return; }
    setLeads((current) => current.map((lead) => lead.id === selectedLead.id ? { ...lead, notes } : lead));
    setSelectedLead({ ...selectedLead, notes });
    setMessage("Observações salvas no perfil da cliente.");
  }
    ...
    <main className="admin-page">
      ...
      {activeSection === "services" && <section className="admin-panel">
        ...
        <form className="new-service-form" onSubmit={createService}>
          ...
          <label>Preço de referência (opcional)<input inputMode="decimal" value={newServicePrice} onChange={(event) => setNewServicePrice(event.target.value.replace(/[^\d,.-]/g, ""))} placeholder="Definido após avaliação" /></label>
      ...
      <section className="admin-panel split-panel">
        <div>
          ...
          <form className="slot-form" onSubmit={createSlot}>
            <label>Procedimento<select required value={slotServiceId} onChange={(event) => { setSlotServiceId(event.target.value); setSlotProfessionalId(""); }}>{activeServices.map((service) => <option key={service.id} value={service.id}>{service.name} · valor após avaliação</option>)}</select></label>
      ...
      <section className="admin-panel">
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {confirmedBookings.length === 0 && <tr><td colSpan={7} className="empty-state">Nenhum agendamento confirmado por pagamento.</td></tr>}
              {confirmedBookings.map((booking) => <tr key={booking.id}><td><b>{booking.leads?.name ?? "Cliente"}</b><small>{booking.leads?.phone ?? ""}</small><small>{booking.leads?.email ?? "Sem e-mail"}</small></td><td>{booking.services?.name ?? "Procedimento"}</td><td>{booking.professional?.full_name ?? "Equipe"}</td><td>{booking.slots ? formatDate(booking.slots.starts_at) : "—"}</td><td><span className={`payment-status ${booking.payments?.[0]?.status ?? "pending"}`}>{booking.payments?.[0]?.status === "paid" ? "Pago" : "—"}</span></td><td><select value={booking.status} onChange={(event) => updateBookingStatus(booking.id, event.target.value)}>{Object.entries(bookingStatus).filter(([value]) => !["pending", "awaiting_payment", "confirmed"].includes(value) || value === booking.status).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></td><td>{booking.leads?.phone ? <a href={`https://wa.me/55${booking.leads.phone.replace(/^55/, "")}`} target="_blank" rel="noreferrer">WhatsApp ↗</a> : "—"}</td></tr>)}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="admin-panel settings-summary">
        ...
        {settings && <form className="settings-form" onSubmit={saveSettings}>
          <label>Sinal fixo (R$)<input type="number" min="1" step="0.01" value={(settings.fixed_deposit_cents / 100).toFixed(2)} onChange={(event) => setSettings({ ...settings, fixed_deposit_cents: Math.round(Number(event.target.value) * 100) })} /><small>Crédito abatido do valor final do procedimento.</small></label>
